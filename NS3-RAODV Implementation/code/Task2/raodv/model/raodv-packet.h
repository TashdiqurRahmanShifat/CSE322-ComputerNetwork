...
class RevRreqHeader : public Header
{
  public:
    ...
    TypeId GetInstanceTypeId() const override;
    uint32_t GetSerializedSize() const override;
    void Serialize(Buffer::Iterator start) const override;//Converts an object's internal state (its fields or members) into a sequence of bytes.
    uint32_t Deserialize(Buffer::Iterator start) override;//The reverse process of serialization.Reconstructs the original object from the serialized byte stream.
    void Print(std::ostream& os) const override;
     ...
     * \return the destination sequence number
     */
    uint32_t GetDstSeqno() const//Helps arrange received data in the correct order
    {
        return m_dstSeqNo;
    }
...
std::ostream& operator<<(std::ostream& os, const RevRreqHeader&);
