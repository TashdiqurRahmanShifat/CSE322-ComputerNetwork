...
GetNodeIdFromContext(std::string context)
{
    ...
    return std::stoul(context.substr(n1 + 1, n2 - n1 - 1));//Extracts the node ID from the context string, which is passed as an argument to tracer callbacks
    //A context like "/NodeList/1/DeviceList/0" would extract 1 as the Node ID
}
...
main(int argc, char* argv[])
{
    ...
    //std::string transport_prot = "TcpScalable";
    double error_p = 0.0;
    std::string bandwidth = "2Mbps";
    //std::string bandwidth = "2005103Mbps";
    ...
    cmd.AddValue("transport_prot",
                 ...
                 "TcpLp, TcpDctcp, TcpCubic, TcpBbr, TweakTcpScalable",
                 transport_prot);
    ...
    uint32_t ip_header = temp_header->GetSerializedSize();//to get the size of the IPv4 header in bytes
    ...
    //monitor->SerializeToXmlFile ("tcp-scalable-flowmon.xml", true, true);

    std::cout << "TCP Port:" << transport_prot<< std::endl;
    std::cout << "Bandwidth:" << bandwidth<< std::endl;
    std::cout << "Flows:" << num_flows<< std::endl;
    Simulator::Destroy();
    return 0;
}
